...
def parse_args():
    ...
    parser.add_argument('--num_category', default=40, type=int, choices=[8, 10, 15, 40],  help='training on ModelNet10/40')
    ...
    parser.add_argument('--train_data_path', type=str, default='Data/train/modelnet10_train_data.npy', help='training data path')
    parser.add_argument('--train_label_path', type=str, default='Data/train/modelnet10_train_labels.npy', help='test data path')
...
def main(args):
    ...
    train_X, train_y = load_data(args.main_path, args.train_data_path, args.train_label_path)
    test_X, test_y = load_data(args.main_path, args.test_data_path, args.test_label_path, mode='test')
    train_X, train_y = train_X.astype(np.float32), train_y.astype(np.int64)
    test_X, test_y = test_X.astype(np.float32), test_y.astype(np.int64)
    
    default_transform = transforms.Compose([Normalize(), toTensor()])
    train_transform = transforms.Compose([Normalize(), RandomNoise(), RandomRotate(), toTensor()])
    
    train_dataset = ModelNetDataLoader(train_X, train_y, transform=train_transform)
